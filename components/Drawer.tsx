...
import SpeedDialNewTask from './SpeedDialNewTask';
import Timer from './Clock/Timer';
import { AlertConfig, ICardPomodoroHist, INotif } from '../interfaces/globals';
import PomodorosList from './Clock/PomodorosList';
...
export default function DrawerX(notif: INotif) {
    const theme = useTheme();
    const [open, setOpen] = React.useState(true);
    const [urlEmbed, setUrlEmbed] = React.useState('https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/playlists/265917916&color=%23ff5500&auto_play=false&hide_related=true&show_comments=false&show_user=false&show_reposts=false&show_teaser=false&visual=false');
    const [serachInput, setSearchInput] = React.useState('')
    const [histoyPomodoros, setHistoryPomodoros] = React.useState<Array<ICardPomodoroHist>>([])
        ...
        <Box sx={{ display: 'flex' }}>
            ...
            <AppBar position="fixed" open={open} >
                <Toolbar>
                    ...
                    <SearchAppBar search={function (input: string) {
            ...
            >
                <div style={{ padding: 10, alignContent: "center", alignItems: "center" }}>
                    <Button variant='text' onClick={() => setOpen(false)} startIcon={theme.direction === 'ltr' ? <ChevronLeftIcon /> : <ChevronRightIcon />}>
                        Close
                    </Button>
                </div>
                <Divider />
                ...
                <List >
                    {[{
                        title: 'All tasks',
                        icon: <PublicIcon sx={{ color: theme.palette.common.white }} />
                    },
                    {
                        title: 'Work',
                        icon: <BusinessCenterIcon sx={{ color: theme.palette.common.white }} />
                    }
                        ,
                    {
                        title: 'Study',
                    ...
                    {
                        title: 'Writing',
                        icon: <MenuBookIcon sx={{ color: theme.palette.common.white }} />

                    }
                    ].map((Tags, index) => (
                        <ListItem key={index} disablePadding>
                            <ListItemButton
                                onClick={() => setSearchInput(Tags.title)}
                                sx={{ bgcolor: serachInput === Tags.title || (serachInput === '' && Tags.title === "All tasks") ? theme.palette.primary.main : "transparent" }}
                            >
                                <ListItemIcon>
                                    {Tags.icon}
                                </ListItemIcon>
                                <ListItemText primary={<Typography sx={{ color: theme.palette.common.white }}>{Tags.title}</Typography>} />
                            </ListItemButton>
                        </ListItem>
                    ))}
                </List>
                <div style={{ paddingLeft: 10, paddingRight: 10, alignContent: "center", alignItems: "center" }}>
                    <Box sx={{ display: 'flex', alignItems: 'flex-end', marginLeft: 2, marginRight: 2 }}>
                        <React.Fragment>
                            <Tooltip title={"Paste your url"}>
                                <OpenInBrowserIcon sx={{ color: theme.palette.common.white, mr: 1, my: 0.5 }} />

                            </Tooltip>
                            <TextField
                                id="input-with-sx"
                                sx={{ color: "white", textAlign: "center" }}
                                label="Your embedding link"
                                variant="standard"
                                InputLabelProps={{
                                    style: { color: "white" },
                                }}
                                onChange={
                                    (event: React.ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => setUrlEmbed(event.target.value)
                                } />
                        </React.Fragment>
                    </Box>
                    <iframe
            ...
            <Main open={open}>
                ...
                <Grid container columns={{ xs: 2, sm: 7 }}>
                    <Grid item sx={{ p: 1 }} sm={4} key={"pomodoro-area"}>
                        <Timer alert={(params) => notif.alert(params)} window={function (params: number): void {
                            notif.window(params);
                        }} finishPomodoro={function (params: ICardPomodoroHist): void {
                            setHistoryPomodoros([...histoyPomodoros, { ...params }])
                        }} />
                        <PomodorosList list={histoyPomodoros} />

                    </Grid>
                    <Grid item sx={{ p: 1 }} xs={2} sm={3} key={"new-task-area"} >
                        <SpeedDialNewTask searchTask={serachInput} />
                    </Grid>
                </Grid>
            </Main>
        </Box>
